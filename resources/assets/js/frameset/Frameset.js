export function initClassicFrameset()
{
    ...
    interact('.postTreeContainer')
        .resizable({
            edges: { top: '.defaultHeader' }
        })
        .on('resizemove', function (event) {
            let target = event.target;
            let x = (parseFloat(target.getAttribute('data-x')) || 0);
            let y = (parseFloat(target.getAttribute('data-y')) || 0);
            let postContainerEl = document.querySelectorAll('.postContainer');
            let threadListContainerEl = document.querySelectorAll('.threadViewContainer')[0];
            let postTreeViewHeight = calculateElementHeightPercent(event.rect.height);

            let postHeightPxNum = parseInt(getComputedStyle(postContainerEl[0])['height'].replace('px', ''));
            let postHeight = calculateElementHeightPercent(postHeightPxNum);
            let threadListHeight = 100 - postTreeViewHeight - postHeight;



            if(threadListHeight<1 || postTreeViewHeight<1)
                return;
            ...
            // target.style.height = threadViewHeight + 'vh';
            $(target)
                .css('height', postTreeViewHeight + 'vh')
                .css('min-height', postTreeViewHeight + 'vh');

            // postTreeContainerEl.style.height = postTreeHeight + 'vh';
            $(threadListContainerEl)
                .css('height', threadListHeight + 'vh')
                .css('min-height', threadListHeight + 'vh');
